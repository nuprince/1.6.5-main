defmodule SpadesGame.GameUI do
  ...
  defstruct [:game, :game_name, :options, :created_at, :status, :waiting, :seats, :when_seats_full]
...
def leave(gameui, userid) do
  seats =
    gameui.seats
    |> Enum.map(fn {position, seat} ->
      if seat.sitting == userid do
        # Player leaves, a bot takes over
        new_seat = GameUISeat.leave(seat)
        new_seat = GameUISeat.bot_sit_if_empty(new_seat)
        {position, new_seat}
      else
        {position, seat}
      end
  ...
  updated_gameui = %{gameui | seats: seats}
  updated_gameui
  |> checks  # Update the game state with the new seating arrangement
end
